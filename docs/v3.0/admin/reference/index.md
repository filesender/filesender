...
Version 3.0 supports a maintenance mode. This allows to interrupt the service for database upgrade or even server restart without breaking uploads.  When switched on:
...
These uid, name, and email are found using the filesender config.php settings auth_sp_saml_email_attribute https://docs.filesender.org/filesender/v3.0/admin/configuration/#auth_sp_saml_email_attribute and the like. 
...
There are two places where templates are used to generate content in
FileSender: (1) web pages are generated by the php code in the
templates directory, and (2) email messages are generated by the
templates in subdirectories of the language directory.

The templates in the templates directory are normal php code. The
templates in the language directory are processed using an internal
template processing engine. The language of this internal engine is ad
hoc and not formalized. The language does not support nesting, for
example you can not next `if` statements inside other `if` statements.

At times various "objects of interest" are passed to the email
template which can then perform conditional computation or other logic
using those objects. For example the `report_inline.mail.php` page
will be passed a "target" object and can tell if something is a
transfer using logic like the below.

```
{if:target.type == "Transfer"} 
```

Each release will mention if the files in the templates have been
updated. This will allow you to merge any customization you may have
made to those files into the updated content.

The mail.php templates are imported each time from the poeditor
language translations. The content in poeditor is considered to be the
master copy of these translations.
...
Describe how the database is initialised for version 3.0 (from classes definiation) and how it's updated automatically!
