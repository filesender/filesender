...
class EncryptionTest extends BasisTest {
    ...
    private function uploadEncrypted() {
        ...
        $this->byId("encryption_password")->clear();
        $this->byId("encryption_password")->value("3mMI3JJSpIzDi7dNGZRokTQ28Yc7gggqfVvUTDDTYPI=");
...
//        $this->assertTrue( false,'waitForAndEnsureCssElementContains(invalid response) v ' . $v . ' selector ' . $selector );
        ...
        $rv = $this->waitUntil(function() use ($test,$selector,$needle){
            try {
                $v = $this->byCssSelector($selector)->text();
    ...
    private function downloadEncrypted() {
        ...
        $this->byCssSelector(".expand")->click();

        // $this->waitForCssElement('.transfer-download');
        // sleep(1);

        $this->byCssSelector(".download_href")->click();
        sleep(2);

        
        // click download
        $this->byCssSelector(".download")->click();
        sleep(5);
        
        // set password
        $this->byCssSelector(".ui-dialog-content.ui-widget-content .wide")->value("3mMI3JJSpIzDi7dNGZRokTQ28Yc7gggqfVvUTDDTYPI=");
        
        // click ok
        $this->byCssSelector(".ui-dialog .ui-dialog-buttonpane .ui-dialog-buttonset .ui-button")->click();
        //        sleep(20);
        // $this->waitForAlert();
        //
        // try{
