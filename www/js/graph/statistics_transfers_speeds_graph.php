...
$sql =
   ...
   .((!$idp) ?
     ...
     :
        '  (SELECT MAX(size/(UNIX_TIMESTAMP(transferssizeview.made_available)-UNIX_TIMESTAMP(transferssizeview.created)))/1048576 FROM transferssizeview LEFT JOIN '.call_user_func('Authentication::getDBTable').' ON transferssizeview.userid='.call_user_func('Authentication::getDBTable').'.id WHERE '.call_user_func('Authentication::getDBTable').'.saml_user_identification_idp = :idp AND DATE(transferssizeview.created) <= Date.date AND DATE(transferssizeview.expires) >= Date.date AND transferssizeview.options LIKE \'%\\"encryption\\":false%\') as Unencrypted, '
       .'  (SELECT MAX(size/(UNIX_TIMESTAMP(transferssizeview.made_available)-UNIX_TIMESTAMP(transferssizeview.created)))/1048576 FROM transferssizeview LEFT JOIN '.call_user_func('Authentication::getDBTable').' ON transferssizeview.userid='.call_user_func('Authentication::getDBTable').'.id WHERE '.call_user_func('Authentication::getDBTable').'.saml_user_identification_idp = :idp AND DATE(transferssizeview.created) <= Date.date AND DATE(transferssizeview.expires) >= Date.date AND transferssizeview.options LIKE \'%\\"encryption\\":true%\') as Encrypted '
   )
   .'FROM '
