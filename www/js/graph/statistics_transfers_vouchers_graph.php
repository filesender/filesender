...
$sql =
   ...
   .(($idp===false) ?
     ...
     :
        '  (SELECT COUNT('.call_user_func('Transfer::getDBTable').'.id) FROM '.call_user_func('Transfer::getDBTable').' LEFT JOIN '.call_user_func('Authentication::getDBTable').' ON '.call_user_func('Transfer::getDBTable').'.userid='.call_user_func('Authentication::getDBTable').'.id WHERE '.call_user_func('Authentication::getDBTable').'.saml_user_identification_idp = :idp AND DATE('.call_user_func('Transfer::getDBTable').'.created) <= Date.date AND DATE('.call_user_func('Transfer::getDBTable').'.expires) >= Date.date) as transfers, '
       .'  (SELECT COUNT('.call_user_func('Guest::getDBTable').'.id) FROM '.call_user_func('Guest::getDBTable').' LEFT JOIN '.call_user_func('Authentication::getDBTable').' ON '.call_user_func('Guest::getDBTable').'.userid='.call_user_func('Authentication::getDBTable').'.id WHERE '.call_user_func('Authentication::getDBTable').'.saml_user_identification_idp = :idp AND DATE('.call_user_func('Guest::getDBTable').'.created) <= Date.date AND DATE('.call_user_func('Guest::getDBTable').'.expires) >= Date.date) as guests '
    )
   .'FROM '
